...
import { each, FormPath, instOf, isFn, isStr, map } from '../shared';
import { compile, registerCompiler, shallowCompile, silent } from './compiler';
import { enablePolyfills, reducePatches, registerPatches, registerPolyfills } from './patches';
import { registerTypeDefaultComponents, registerVoidComponents } from './polyfills';
...
> implements ISchema
{
  ...
  definitions?: Record<
    string,
    Schema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>
  >;
  properties?: Record<
    string,
    Schema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>
  ...
  constructor(
    json: ISchema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>,
    parent?: Schema,
  ) {
    if (parent) {
      this.parent = parent;
  ...
  addProperty = (
    key: SchemaKey,
    schema: ISchema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>,
  ) => {
  ...
  setAdditionalProperties = (
    properties: ISchema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>,
  ) => {
  ...
  setAdditionalItems = (
    items: ISchema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>,
  ) => {
    if (!items) return;
  ...
  mapProperties = <T>(
    callback?: (
      schema: Schema<Decorator, Component, DecoratorProps, ComponentProps, Pattern, Display, Validator, Message>,
      key: SchemaKey,
      index: number,
    ) => T,
  ): T[] => {
  ...
  static getOrderProperties = (schema: ISchema = {}, propertiesName: keyof ISchema = 'properties') => {
