import { compile, patchCompile, patchSchemaCompile, registerCompiler, shallowCompile, silent } from '../compiler';
...
test('compile', () => {
  ...
  expect(compiledSchema.properties?.['aa']['x-component-props']).toEqual('{{123}}');
  const toJSable = {
    toJS() {
      return {
        aa: 123,
