...
const Input: React.FC<{
  ...
  return <input data-testid="input" {...others} value={value || ''} onChange={onChange} />;
};
...
describe('markup schema field', () => {
  ...
  test('props children', () => {
    ...
    const { queryByTestId } = render(
      <FormProvider form={form}>
        <SchemaField>
          <SchemaField.Void x-component="Text" x-component-props={{ children: 'props' }} />
        </SchemaField>
      </FormProvider>,
    );
    ...
    const CustomObject2: React.FC = () => {
      ...
      return (
        <div data-testid="only-properties">
          <RecursionField name={schema.name} basePath={field.address} schema={schema} onlyRenderProperties />
        </div>
      );
    };
    const SchemaField = createSchemaField({
...
test('expression x-content', async () => {
  ...
  const { queryByTestId } = render(
    <FormProvider form={form}>
      <SchemaField>
        <SchemaField.String name="aaa" x-component="Wrapper" x-content="{{child}}" />
      </SchemaField>
    </FormProvider>,
  );
  ...
  const { queryByText } = render(
    <FormProvider form={form}>
      <SchemaField>
        ...
        <SchemaField.String name="bbb" x-component="BBB" x-visible="{{$form.values.aaa === 123}}" />
      </SchemaField>
    </FormProvider>,
  );
  ...
  const { queryByText } = render(
    <FormProvider form={form}>
      <SchemaField>
        ...
        <SchemaField.String name="bbb" x-component="Text" x-value="{{$form.values.aaa * 10}}" />
      </SchemaField>
    </FormProvider>,
  );
...
test('schema x-validator/required', async () => {
  ...
  render(
    <FormProvider form={form}>
      <SchemaField>
        <SchemaField.String name="input" required x-validator="email" x-component="Input" />
      </SchemaField>
    </FormProvider>,
  );
  await waitFor(() => {
...
test('void field children', async () => {
  ...
  const SchemaField = createSchemaField({
    components: {
      Button: (props) => <div data-testid="btn">{props.children || 'placeholder'}</div>,
    },
  });
