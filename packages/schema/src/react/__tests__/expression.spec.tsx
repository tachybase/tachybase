...
test('expression scope', async () => {
  const Container = (props) => {
    return <ExpressionScope value={{ $innerScope: 'this is inner scope value' }}>{props.children}</ExpressionScope>;
  };
    ...
    <FormProvider form={form}>
      <SchemaField scope={{ $outerScope: 'this is outer scope value' }}>
        <SchemaField.Void x-component="Container">
          <SchemaField.String name="input" x-component="Input" x-value="{{$innerScope + ' ' + $outerScope}}" />
        </SchemaField.Void>
      </SchemaField>
    </FormProvider>,
  );

  expect(getByTestId('test-input').textContent).toBe('this is inner scope value this is outer scope value');
});
