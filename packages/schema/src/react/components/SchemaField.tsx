...
export function createSchemaField<Components extends SchemaReactComponents>(
  ...
  function SchemaField<Decorator extends JSXComponent, Component extends JSXComponent>(
    ...
    const renderMarkup = () => {
      ...
      return render(<SchemaMarkupContext.Provider value={schema}>{props.children}</SchemaMarkupContext.Provider>);
    };
    ...
    return (
      <SchemaOptionsContext.Provider value={options}>
        <SchemaComponentsContext.Provider value={lazyMerge(options.components, props.components)}>
  ...
  function MarkupRender(props: any) {
    ...
    if (parent.type === 'object' || parent.type === 'void') {
      ...
      return <SchemaMarkupContext.Provider value={schema}>{renderChildren()}</SchemaMarkupContext.Provider>;
    } else if (parent.type === 'array') {
      const schema = appendArraySchema(props);
      return (
        <SchemaMarkupContext.Provider value={Array.isArray(schema) ? schema[0] : schema}>
          {props.children}
