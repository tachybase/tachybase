...
import { autorun, batch, observable } from '../../reactive';
...
test('selfValidate/errors/warnings/successes/valid/invalid/validateStatus/queryFeedbacks', async () => {
  ...
  expect(field3.feedbacks).toEqual([{ code: 'EffectError', messages: ['error2'] }]);
  expect(
...
test('query value with sibling path syntax', () => {
  ...
  attach(
    form.createField({
      ...
      reactions: [
        (field) => {
          fn(field.query('.textarea').value(), field.query('.textarea').initialValue());
        },
      ],
    }),
  );
...
test('validateFirst', async () => {
  ...
  const aaValidate = vi.fn(() => 'aaError');
  const aa = attach(
    form.createField({
      name: 'aa',
      validateFirst: true,
  ...
  await aa.onInput('aa');
  const bbValidate = vi.fn(() => 'bbError');
  const bb = attach(
    form.createField({
      name: 'bb',
  ...
  await bb.onInput('bb');
  const ccValidate = vi.fn(() => 'ccError');
  const cc = attach(
    form.createField({
      name: 'cc',
