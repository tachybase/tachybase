...
import { onFieldValueChange, onFormInitialValuesChange, onFormValuesChange } from '../effects';
...
test('array field children state exchanges', () => {
  ...
  expect(Object.keys(form.fields).sort()).toEqual(['array', 'array.0.value', 'array.1.value', 'array.other']);
  array.pop();
...
test('nest array remove', async () => {
  ...
  expect(form.fields['metrics.0.content.0.attr']).not.toBeUndefined();
  expect(form.initialValues.metrics?.[1]?.content?.[0]?.attr).not.toBeUndefined();
});
...
test('record: find array nest field record', () => {
  ...
  expect(field1.records).toEqual([{ a: { b: { c: 1, d: 1 } } }, { a: { b: { c: 2, d: 2 } } }]);
