...
import { batch, observable } from '../../reactive';
import { onFieldValidateStart, onFieldValueChange, onFormInitialValuesChange, onFormValuesChange } from '../effects';
...
test('createField/createArrayField/createObjectField/createVoidField', () => {
  ...
  const void_ = attach(form.createVoidField({ name: 'void', basePath: 'parent' }));
  const void2_ = attach(form.createVoidField({ name: 'void', basePath: 'parent' }));
...
test('query', () => {
  ...
  expect(form.query('object.*').map((field) => field.path.toString())).toEqual(['object.void', 'object.normal']);
