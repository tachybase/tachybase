...
import { every, find, findIndex, includes, map, move, reduce, some, toArr } from '../array';
import { lowerCase } from '../case';
import { isFn, isHTMLElement, isMap, isNumberLike, isReactElement, isSet, isWeakMap, isWeakSet } from '../checkers';
...
test('compare', () => {
  ...
  expect(isEqual(new URL('https://aa.test'), new URL('https://aa.test'))).toBeTruthy();
  expect(
    isEqual(
      {
        $$typeof: true,
...
describe('deprecate', () => {
  test('deprecate', () => {
    ...
    const deprecatedFn = vi.fn(deprecate(test, 'Some.Deprecated.Api', 'some deprecated error'));
    ...
    // arguments - string
    const testDeprecatedFn = vi.fn(() => deprecate('Some.Deprecated.Api', 'some deprecated error'));
    testDeprecatedFn();
    expect(testDeprecatedFn).toHaveBeenCalledTimes(1);

    // arguments - empty string
    const testDeprecatedFn2 = vi.fn(() => deprecate('Some.Deprecated.Api'));
    testDeprecatedFn2();
...
describe('shared Subscribable', () => {
  test('Subscribable', () => {
    const cb = vi.fn((payload) => payload);
    ...
    const customNotify = vi.fn((payload) => {
      console.info(payload);
      return false;
    });
...
test('applyMiddleware', async () => {
  ...
  applyMiddleware(0, [(num: number, next) => next(num + 1), () => '123', (num: number, next) => next(num + 1)]).then(
    resolved,
  );
  await sleep(16);
