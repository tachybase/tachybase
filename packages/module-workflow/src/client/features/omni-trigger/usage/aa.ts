...
import { formatCurrency, formatQuantity } from '@tachybase/utils/client';
...
import {
  ...
  Itemcategory,
  RulesNumber,
} from '@hera/rental/constants';
import Item from 'antd/es/list/Item';
import dayjs from 'dayjs';

export default async function (data, context) {
  const ctx = context.httpContext;
  return calculate(data.settlementAbout[0][0], data.settlementsId);
}
...
const calculate = (settlementAbout, settlementsId) => {
  let createLeasDatas = [];
  let createFeesDatas = [];
  ...
  let record_start_end = new Date();
  let tax_rate = 0;
  //统计出库数量
  settlementAbout.records?.forEach((recordItems) => {
    recordItems['movement'] = recordItems.record_contracts[0].inOut;
    recordItems['category'] = '0';
    if (dayjs(recordItems.date).isBefore(record_start_end)) {
      record_start_end = recordItems.date;
    }
              ...
              if (rule.conversion_logic_id === ConversionLogics.ActualWeight) {
                if (item.weight_items.length) {
                    ...
                    if (recordItem) {
                      ...
                      if (isCount) {
                        ...
                        const data = {
                          ...
                          movement: recordItem.weight < 0 ? movement : item.movement, //出入库状态
                          date: item.date, //时间
                          name: rule.comment ?? '', //名称
                          label: rule.comment ?? '',
                          category: item.category || '0', //费用类别
                        ...
                        if (productLength > 1) {
                          ...
                          const isBeforeDay = dayjs(item.date).isBefore(settlementAbout.free_date?.[0].value);
                          const isBetweenDay = dayjs(item.date).isBetween(
                            settlementAbout.free_date?.[0].value,
                            settlementAbout.free_date?.[1].value,
                            'day',
                            '[]',
                          );
                          if (isFreeDate && (isBeforeDay || isBetweenDay)) {
                            const freeData = {
                              ...data,
                            ...
                            const days = isBetweenDay
                              ? afterDays(
                                  settlementAbout.contracts.calc_type,
                                  item.movement,
                                  item.date,
                                  dayjs(settlementAbout.free_date[1].value).startOf('day'),
                                ) - 1
                              : l.diff(r, 'day');
                  ...
                  if (recordItem) {
                    ...
                    const data = {
                      settlement_id: settlementsId, //合同ID
                      movement: item.weigh < 0 ? movement : item.movement, //出入库状态
                      date: item.date, //时间
                ...
                item.record_items?.forEach((recordItem) => {
                  if (recordItem) {
                    if (
                      ...
                      const data = {
                        settlement_id: settlementsId, //合同ID
                        movement: recordItem.count < 0 ? movement : item.movement, //出入库状态
                        date: item.date, //时间
                                      ...
                                      recordItem.record_item_fee_items.forEach((value) => {
                                        if (value.product_id === rulefee.fee_product_id) {
                                          ...
                                          createFeesDatas.push({
                                            ...
                                            date: item.date,
                                            name: recordItem.product.name + '-' + spec,
  ...
  //计算完结单损耗
  createLeasDatas = calcLoss(createLeasDatas, outMovement);
  //计算报价数据
  settlementAbout.records?.forEach((recordItems) => {
    if (
      dayjs(recordItems.date).isBetween(settlementAbout.start_date, settlementAbout.endDate, 'day', '[]') &&
      recordItems.price_items.length &&
      recordItems.record_contracts[0].is_price
    ) {
      const categoryList = [];
      recordItems.record_items?.forEach((item) => {
        if (!item?.product) return;
        const price_item = recordItems.price_items.find(
          (priceItem) =>
            priceItem?.product_id === item?.product.id || priceItem?.product_id - 99999 === item?.product?.category_id,
        );
        if (!price_item?.product_id) return;
        const productCategory = price_item.product_id > RulesNumber ? 'category' : 'product';
        const movement = recordItems.movement === '-1' ? '1' : '-1';
        const data = {
          settlement_id: settlementsId, //合同ID
          movement: recordItems.movement, //出入库状态
          date: recordItems.date, //时间
          category: '无物赔偿', //费用类别
          is_excluded: false,
          unit_name: '',
          productCategory,
          count: 0,
          unit_price: price_item.unit_price || 0,
          item_count: 0,
          amount: 0,
        };

        if (price_item.conversion_logic_id === ConversionLogics.ActualWeight) {
          if (categoryList.includes(price_item.product_id)) return;
          const weight = recordItems.weight_items?.find(
            (item) =>
              item.product_category_id === price_item.product_id - 99999 ||
              item.product_category_id === price_item.product?.category_id,
          )?.weight;
          data.movement = weight < 1 ? movement : data.movement;
          data.unit_name = '吨';
          data.item_count = weight * Number(recordItems.movement);
          data.count = weight * Number(recordItems.movement);
          data.amount = data.count * (data.unit_price * 1000) * Number(recordItems.movement);
          categoryList.push(price_item.product_id);
        } else {
          price_item['ucl'] = {
            weight_items: price_item.weight_items,
          };
          const { count, unit } = ruleCount(price_item, recordItems, item);
          data.movement = item.count < 1 ? movement : data.movement;
          data.count = count * Number(recordItems.movement);
          data.unit_name = unit;
          data.item_count = (item.count || 0) * Number(recordItems.movement);
          data.amount = data.count * data.unit_price;
        }
        const priceLabel = data.movement === '-1' ? '采购' : '销售';
        data['label'] = item.product.label + '-' + priceLabel;
        data['name'] = item.product.name + '-' + priceLabel;
        createFeesDatas.push(data);
      });
    }
  });
  ...
  summaryPeriod.tax = settlementAbout.contracts.tax_included ? 0 : tax_rate;
  ...
  summaryPeriod.accumulate = settlement.length
    ? settlement.reduce(
        (sum, curr) => sum + (curr.is_accumulate ? 0 : curr.current_expenses),
        summaryPeriod.current_expenses,
      )
    : summaryPeriod.current_expenses;
  return {
    summaryPeriod,
    createDatas,
    categoryHistory: createCategoryDatasItem.history,
    productHistory: createProductDatasItem.history,
    summaryItems,
  };
};
...
/**
 *计算完结损耗
 * @param createLeasDatas
 */
const calcLoss = (createLeasDatas, outMovement) => {
  const countData = {};

  createLeasDatas.forEach((leasData) => {
    if (leasData.movement === outMovement && leasData.category === '0') {
      if (countData[leasData.label]) {
        countData[leasData.label].count = countData[leasData.label].count + leasData.count;
      } else {
        countData[leasData.label] = {
          ...leasData,
          lossCount: leasData.count,
        };
      }
    }
  });
  createLeasDatas.forEach((leasData) => {
    if (leasData.is_end && leasData.category === '0') {
      const loss = countData[leasData.label].count * leasData.loss;
      if (leasData.count - loss > 0) {
        createLeasDatas.push({
          ...leasData,
          itemCount: 0,
          count: leasData.count - loss,
          category: '损耗',
        });
      }
      leasData.count = leasData.count - loss > 0 ? leasData.count - loss : 0;
    }
  });

  return createLeasDatas;
};
...
const calcDiffer = (type, start, end, shortest_day, differData, pushDatas, entMovement, free_date, recordItem) => {
  ...
  if (differDay > 0 && shortest_day - differDay > 0) {
    ...
    if (free_date?.length === 2 && (isBeforeDay || isBetweenDay)) {
...
const screenData = (data, settlementAbout, type) => {
  ...
  data.forEach((value) => {
    ...
    const inCount =
      value.movement === '1'
        ? value.item_count < 0
          ? formatNumber(value.item_count) * -1
          : formatNumber(value.item_count)
        : 0;
    const outCount =
      value.movement === '-1'
        ? value.item_count < 0
    ...
    if (dayjs(value.date).isBefore(settlementAbout.start_date)) {
      if (!history[historyKey]) {
        ...
        if (value.movement === '-1') {
          history[historyKey].out_number = formatNumber(history[historyKey].out_number) + outCount;
        } else if (value.movement === '1') {
          history[historyKey].in_number = formatNumber(history[historyKey].in_number) + inCount;
        }
      }
    } else {
      if (!list[listKey]) {
        ...
        if (value.movement === '-1') {
          list[listKey].out_number = formatNumber(list[listKey].out_number) + outCount;
        } else if (value.movement === '1') {
          list[listKey].in_number = formatNumber(list[listKey].in_number) + inCount;
        }
      }
    }
  });
