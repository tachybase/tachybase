...
function isBuiltin(moduleName) {
  if (Reflect.has(module_1.default, 'isBuiltin') && typeof module_1.default.isBuiltin === 'function') {
    return module_1.default.isBuiltin(moduleName);
  }
  const moduleNameWithoutPrefix = moduleName.startsWith('node:') ? moduleName.slice(5) : moduleName;
...
async function findCommonJunctionPoint(file, realFile) {
  ...
  while (true) {
    ...
    if (file === path_1.default.parse(file).root || realFile === path_1.default.parse(realFile).root) {
...
class Walker {
  ...
  appendRecord({ file, store }) {
    ...
    if (store === common_1.STORE_BLOB || store === common_1.STORE_CONTENT || store === common_1.STORE_LINKS) {
  ...
  async stepActivate(marker, derivatives) {
    ...
    if (!marker.public && !marker.toplevel && this.params.publicPackages) {
      marker.public =
        this.params.publicPackages[0] === '*' || (!!name && this.params.publicPackages.indexOf(name) !== -1);
    }
  ...
  async stepDerivatives_ALIAS_AS_RELATIVE(record, marker, derivative) {
    ...
    try {
      ...
      const level = debug ? 'debug' : 'warn';
      log_1.log[level](`Cannot stat, ${exception.code}`, [file, `The file was required from '${record.file}'`]);
    }
    if (stat && stat.isFile()) {
      await this.appendBlobOrContent({
        file,
  ...
  async stepDerivatives_ALIAS_AS_RESOLVABLE(record, marker, derivative) {
    ...
    if (failure) {
      ...
      const mainNotFound =
        ...
        !((_b = (_a = newPackages[0].marker) === null || _a === void 0 ? void 0 : _a.config) === null || _b === void 0
          ? void 0
          : _b.main);
      ...
      if (mainNotFound) {
        ...
        log_1.log[level](message, [`%1: ${newPackages[0].packageJson}`, `%2: ${record.file}`]);
      } else {
    ...
    if (newPackageForNewRecords) {
      if (strictVerify) {
        (0, assert_1.default)(
          newPackageForNewRecords.packageJson === (0, common_1.normalizePath)(newPackageForNewRecords.packageJson),
        );
      }
      await this.appendBlobOrContent({
