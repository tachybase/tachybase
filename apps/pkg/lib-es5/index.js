...
const { hostArch, hostPlatform, isValidNodeRange, knownArchs, knownPlatforms, toFancyArch, toFancyPlatform } =
  pkg_fetch_1.system;
...
function fabricatorForTarget({ nodeRange, arch }) {
  ...
  if (hostArch !== arch && (hostPlatform === 'linux' || hostPlatform === 'alpine')) {
...
async function exec(argv2) {
  ...
  // _
  if (!argv._.length) {
    throw (0, log_1.wasReported)('Entry file/directory is expected', ['Pass --help to see usage information']);
  }
  if (argv._.length > 1) {
  ...
  if (inputJson) {
    ...
    if (bin) {
      ...
      if (!(0, fs_1.existsSync)(inputBin)) {
        throw (0, log_1.wasReported)('Bin file does not exist (taken from package.json ' + "'bin' property)", [
          inputBin,
        ]);
      }
    }
  ...
  for (const target of targets) {
    if (target.output && (0, fs_1.existsSync)(target.output)) {
      if ((await (0, promises_1.stat)(target.output)).isFile()) {
        ...
        throw (0, log_1.wasReported)('Refusing to overwrite non-file output', [target.output]);
      }
