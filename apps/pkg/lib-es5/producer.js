...
function nativePrebuildInstall(target, nodeFile) {
  ...
  const packageJson = JSON.parse(
    fs_1.default.readFileSync(path_1.default.join(dir, 'package.json'), { encoding: 'utf-8' }),
  );
  ...
  if (
    ((_a = packageJson.dependencies) === null || _a === void 0 ? void 0 : _a['prebuild-install']) == null &&
    ![`${envPrefix}_binary_host`, `${envPrefix}_binary_host_mirror`, `${envPrefix}_local_prebuilds`].some(
      (i) => i in process.env,
    )
  ) {
    return;
  ...
  const napiVersions =
    (_b = packageJson === null || packageJson === void 0 ? void 0 : packageJson.binary) === null || _b === void 0
      ? void 0
      : _b.napi_versions;
  const options = ['--platform', types_1.platform[target.platform], '--arch', target.arch];
